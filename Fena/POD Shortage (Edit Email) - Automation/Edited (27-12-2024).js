...
async function getOrderNumbers(orderIds) {
    try {
        ...
        let url = `${FRT_PUB_BASE_URL}/shipment-view/sales/v2/orders?source=["orderNumber","customFields"]&filters=${JSON.stringify(filters)}`
        let res = await rp({
            url: url,
            method: "GET",
            json: true,
...
async function main() {
    try {
        ...
        let { orderNumbers, customFields } = await getOrderNumbers(orderIds)
        if (customFields?.some(field => field.fieldKey === "Order Type" && field.value === "STO")) {
            console.log("STO Order")
            ccArray.push("kamaldhyani@fena.com")
            ccArray.push("vinodsharma@fena.com")
            ccArray.push("santosh@fena.com")
        }
