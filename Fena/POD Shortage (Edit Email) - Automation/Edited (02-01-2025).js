...
async function getOrderNumbers(orderIds) {
    try {
        ...
        let url = `${FRT_PUB_BASE_URL}/shipment-view/sales/v2/orders?source=["orderNumber","customFields","externalId"]&filters=${JSON.stringify(filters)}`
        let res = await rp({
            url: url,
            method: "GET",
            json: true,
        ...
        if (res?.length) {
            ...
            let externalIds = res.map((v) => { return v.externalId })
            let customFields = res[0].customFields;
            if (orderNumbers.length) {
                return {
                    orderNumbers: _.uniq(orderNumbers)?.join(","),
                    externalIds: _.uniq(externalIds)?.join(","),
...
async function ensureMailAndCfForPodShortage(cn) {
    try {
        ...
        let invoiceNo = cn?.consignmentNo ?? "N/A"
        let origin = cn?.consigner?.places?.length ? cn?.consigner?.places[0]?.name : "N/A"
        let destination = cn?.consignee?.places?.length ? cn?.consignee?.places[0]?.name : "N/A"
        ...
        for (let delItem of deliveryItems) {
            ...
            let orderId = [delItem.orderMapping.orderId]
            let { customFields, externalIds } = await getOrderNumbers(orderId)
            if (customFields?.some(field => field.fieldKey === "Order Type" && field.value === "STO")) {
                console.log("STO Order")
                if (!CC.includes("kamaldhyani@fena.com")) {
                    CC.push("kamaldhyani@fena.com")
                }
                if (!CC.includes("vinodsharma@fena.com")) {
                    CC.push("vinodsharma@fena.com") 
                }
                if (!CC.includes("santosh@fena.com")) {
                    CC.push("santosh@fena.com")
                }
            }
            if (!shortage) {
                continue;
