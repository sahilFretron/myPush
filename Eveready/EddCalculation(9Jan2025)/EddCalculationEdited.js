...
async function main(shId){
    try{
        ...
        if((origin == "" || !origin || origin == undefined) && (dest == "" || !dest || dest == undefined)){
            let originName = shStages[0].place?.name ?? shStages[0].hub?.name ?? "N/A"
            let destName = shStages[shStages.length - 1].place?.name ?? shStages[shStages.length - 1].hub?.name ?? "N/A"
            let originPartner = await getBPartner(originName)
            let destPartner = await getBPartner(destName)
            origin = originPartner?.[0]?.externalId
            dest = destPartner?.[0]?.externalId
        }
...
async function getBPartner(name){
    try {
        let filters = {
            "type": ["customer"],
            "name": [name]
        ...
        let res = await rp({
            method: 'GET',
            uri: `${FRT_PUB_BASE_URL}/shipment-view/bpartners/partners`,
            headers: {
                'Authorization': token,
        ...
        console.log(`Error getting bpartner ${name}: ${e.message}`)
        return null
    }
}
